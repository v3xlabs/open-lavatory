...
function updateNetworkInfo() {
  const network = networks[walletState.chainId] || { name: 'Unknown Network', symbol: '?' };
...
function updateAccountsList() {
  ...
  if (walletState.accounts.length === 0) {
    accountsListElement.innerHTML = '<div style="text-align: center; color: #666; padding: 20px;">No accounts available</div>';
    return;
  }
...
function updateConnectionStatus() {
  ...
  const statusClass = walletState.isConnected ? 'status-connected' : 'status-disconnected';
