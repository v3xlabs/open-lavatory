...
test.describe('OpenLV Sandbox P2P Connection', () => {
  ...
  test('should establish P2P connection between two sandbox instances', async () => {
    ...
    await test.step('Peer A initializes session', async () => {
      ...
      await expect(peerAPage.locator('[data-testid="connection-url"]')).toBeVisible({ timeout: 10000 });
      
      // Verify status shows connecting state (waiting for wallet or connected)
      await expect(peerAPage.locator('[data-testid="connection-status"]')).toContainText(/Waiting for wallet|Connected|P2P|MQTT/);
    });
    ...
    await test.step('Peer B connects to session', async () => {
      ...
      await expect(peerBPage.locator('[data-testid="connection-status"]')).toContainText(/Connected|P2P.*ready/, { timeout: 30000 });
    });
    ...
    await test.step('Verify connection established on both sides', async () => {
      ...
      await expect(peerAPage.locator('[data-testid="connection-status"]')).toContainText(/Connected|P2P.*ready/, { timeout: 30000 });
      
      // Check Peer B shows connected  
      await expect(peerBPage.locator('[data-testid="connection-status"]')).toContainText(/Connected|P2P.*ready/);
    ...
    await test.step('Test bidirectional messaging', async () => {
      ...
      await expect(peerAPage.locator('[data-testid="message-log"]')).toContainText(/Test sent via/, { timeout: 10000 });
      
      // Wait for message to be received by Peer B
      await expect(peerBPage.locator('[data-testid="message-log"]')).toContainText(/Received/, { timeout: 10000 });
      ...
      await expect(peerBPage.locator('[data-testid="message-log"]')).toContainText(/Test sent via/, { timeout: 10000 });
      
      // Wait for message to be received by Peer A
      await expect(peerAPage.locator('[data-testid="message-log"]')).toContainText(/Received.*Test message/, { timeout: 10000 });
    });
    ...
    await test.step('Verify connection quality', async () => {
      ...
      const peerAStatus = await peerAPage.locator('[data-testid="connection-status"]').textContent();
      const peerBStatus = await peerBPage.locator('[data-testid="connection-status"]').textContent();
  ...
  test('should handle connection failures gracefully', async () => {
    ...
    await test.step('Test invalid connection URL', async () => {
      ...
      await expect(peerBPage.locator('[data-testid="connection-status"]')).not.toContainText(/WebRTC Connected/, { timeout: 5000 });
    });
  });
  ...
  test('should support multiple connection attempts', async () => {
    ...
    const connectionUrl = await peerAPage.locator('[data-testid="connection-url"]').textContent();
