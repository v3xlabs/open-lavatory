  ...
  = (init: SignalingBaseLayer): SignalingLayerFn =>
    async ({
      ...
      const handleReceive = async (payload: string) => {
        try {
          await handleReceiveInner(payload);
        }
        catch (error) {
          const signalError = new SignalDecryptionError({
            cause: error instanceof Error ? error : new Error(String(error)),
          });

          log("Signal: message decryption/parsing error", signalError);
          emitter.emit("error", signalError);
        }
      };

      const handleReceiveInner = async (payload: string) => {
