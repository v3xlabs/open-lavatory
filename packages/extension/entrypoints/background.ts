...
export default defineBackground(() => {
    ...
    try {
      ...
      switch (message.type) {
        ...
        case 'eth_signTypedData_v4':
          return await handleSignTypedData(message.type, message.data, sender.tab?.id);
  ...
  async function handleRequestAccounts(data: { tabId: number }, tabId?: number) {
    ...
    try {
      ...
      await showUserConsentDialog('Connect Wallet', 'Allow this site to access your wallet?');
  ...
  async function handleSendTransaction(data: { transaction: any }, tabId?: number) {
  ...
  async function handlePersonalSign(data: { message: string; address: string }, tabId?: number) {
  ...
  async function handleEthSign(data: { address: string; message: string }, tabId?: number) {
  ...
  async function handleSignTypedData(method: string, data: { params: any[] }, tabId?: number) {
  ...
  async function handleRequestPermissions(data: { permissions: any }, tabId?: number) {
    ...
    try {
      ...
      walletState.permissions = [...walletState.permissions, ...grantedPermissions];
  ...
  async function showUserConsentDialog(title: string, message: string): Promise<boolean> {
