...
(function() {
  ...
  class OpenLVProvider {
    ...
    _handleContentScriptMessage(message) {
      if (message.requestId && this._pendingRequests.has(message.requestId)) {
        const { resolve, reject } = this._pendingRequests.get(message.requestId);
    ...
    async _sendTransaction(transactionObject) {
      return this._sendToContentScript('eth_sendTransaction', { transaction: transactionObject });
    }
    ...
    async _switchChain(chainParams) {
      const result = await this._sendToContentScript('wallet_switchEthereumChain', chainParams);
      if (result.chainId) {
    ...
    async _requestPermissions(permissions) {
      return this._sendToContentScript('wallet_requestPermissions', { permissions });
    }
  ...
  console.log('OpenLV EIP-6963 Provider announced with RDNS:', providerInfo.rdns);
