...
test.describe("OpenLV Sandbox P2P Connection", () => {
  ...
  test.beforeEach(async ({ browser }) => {
    ...
    await expect(
      peerAPage.locator("h1:has-text(\"OpenLV Wallet Demo\")"),
    ).toBeVisible();
    await expect(
      peerBPage.locator("h1:has-text(\"OpenLV Wallet Demo\")"),
    ).toBeVisible();
  });
  ...
  test("should establish P2P connection between two sandbox instances", async () => {
    ...
    await test.step("Peer A initializes session", async () => {
      ...
      await expect(
        peerAPage.locator("[data-testid=\"connection-url\"]"),
      ).toBeVisible({
        timeout: 10_000,
      });
      ...
      await expect(
        peerAPage.locator("[data-testid=\"connection-status\"]"),
      ).toContainText(/Waiting for wallet|Connected|P2P|MQTT/);
    });

    // Step 2: Get connection URL from Peer A
    const connectionUrl
      = await test.step("Extract connection URL", async () => {
    ...
    await test.step("Peer B connects to session", async () => {
      ...
      await expect(
        peerBPage.locator("[data-testid=\"connection-status\"]"),
      ).toContainText(/Connected|P2P.*ready/, { timeout: 30_000 });
    });
    ...
    await test.step("Verify connection established on both sides", async () => {
      ...
      await expect(
        peerBPage.locator("[data-testid=\"connection-status\"]"),
      ).toContainText(/Connected|P2P.*ready/);

      // Verify messaging interface is available on both sides
      await expect(
        peerAPage.locator("[data-testid=\"send-test-button\"]"),
      ).toBeVisible();
      await expect(
        peerBPage.locator("[data-testid=\"send-test-button\"]"),
      ).toBeVisible();
    });
    ...
    await test.step("Test bidirectional messaging", async () => {
      ...
      await expect(
        peerAPage.locator("[data-testid=\"message-log\"]"),
      ).toContainText(/Test sent via/, { timeout: 10_000 });

      // Wait for message to be received by Peer B
      await expect(
        peerBPage.locator("[data-testid=\"message-log\"]"),
      ).toContainText(/Received/, { timeout: 10_000 });
      ...
      await expect(
        peerAPage.locator("[data-testid=\"message-log\"]"),
      ).toContainText(/Received.*Test message/, { timeout: 10_000 });
    });
  ...
  test("should handle connection failures gracefully", async () => {
    ...
    await test.step("Test invalid connection URL", async () => {
      ...
      await expect(
        peerBPage.locator("[data-testid=\"connection-status\"]"),
      ).not.toContainText(/WebRTC Connected/, { timeout: 5000 });
    });
  });
